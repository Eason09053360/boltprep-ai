        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[90] p-4">
            <div className="bg-white p-8 rounded-[2rem] shadow-2xl w-full max-w-md animate-fade-in border border-slate-100">
                ...
                <div className="space-y-6">
                    ...
                    <button onClick={() => { const key = apiKey.trim(); localStorage.setItem("fastframe_key", key); setApiKey(key); setShowSet(false); }} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold hover:bg-slate-800 transition active:scale-95 shadow-xl">儲存並返回</button>
                </div>
            </div>
        </div>
    );
