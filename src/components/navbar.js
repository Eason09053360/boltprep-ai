        ...
        <nav className="fixed top-0 left-0 right-0 glass-nav px-3 py-2 z-50">
            <div className="max-w-6xl mx-auto flex justify-between items-center group">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-black tracking-tighter text-slate-800 italic">BOLTPREP <span className="text-blue-600 font-bold not-italic">AI</span></h1>
                </div>
                <div className="flex gap-2 items-center flex-wrap justify-end">
                    ...
                    <button
                        ...
                        className="px-3 py-1.5 rounded-xl font-bold transition text-sm border bg-white text-slate-700 border-slate-200 hover:bg-slate-50"
                    >
                        回到上次進度
                    </button>
                    ...
                    {isExamMode && (
                        <button onClick={() => setIsExamMode(false)} className="bg-rose-50 text-rose-600 px-4 py-1.5 rounded-xl font-bold border border-rose-100 transition text-sm">離開測驗</button>
                    )}
                    {apiKey && allQuestions.length > 0 && (
                        <button onClick={translateAll} className="bg-green-100 text-green-700 px-4 py-1.5 rounded-xl font-bold border border-green-300 transition text-sm hover:bg-green-200"><i className="fas fa-language mr-2"></i>翻譯全部</button>
                    )}
                    <button onClick={() => setShowSet(true)} className="w-9 h-9 rounded-xl bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition shadow-sm"><i className="fas fa-bolt text-slate-400"></i></button>
                </div>
            </div>
        </nav>
    );
